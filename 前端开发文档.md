...
在选中时，进行一次重绘，并inti绘制层
目前已经优化多层绘制，后续还可以对图片这类闪烁问题进行绘制

### 插入矩阵后切换为选中功能时错误的网络请求bug
插入矩阵后进行切换会出现错误的网络请求

### 在线功能实现
目前在登录之前所有功能为本地功能，将boardId和userId设为-1。
在网络请求时也根据这个来判断是否进行网络请求，
在登录之后将栈中的所有id都进行遍历更新。
### 后台部分开发
目前暂定开发的功能有
浏览用户数据
浏览白板数据
...
### 前端交互逻辑修改
在用户登录之前，所有的数据都都不与后端进行交互，在用户登录之后，获取一个boardId，再将现在已经绘制的笔迹保存到数据库

如果不进行交互 撤销功能的实现问题 

设置初始boardId为-1，用户若没登陆，userId也为-1，在进行网络交互时，检查到两者其中一个为-1则不进行网络交互。单纯将数据保存在pathArr中，其中数据的userId和boarId都为-1

若有userId，有token保留，直接获取一个boardId，若token过期，则提示过期，并将userId设置为-1

在登录之后，获取一个boardId，并且遍历pathArr，更改数据的userId和boardId
